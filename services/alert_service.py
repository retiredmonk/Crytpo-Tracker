...
def check_alerts(conn, cursor, coins):
    now = int(time.time())
    ...
    for coin in coins:
        ...
        if should_alert(price, last_alert_time, now):
            ...
            update_alert_state(cursor, conn, coin, price, now)